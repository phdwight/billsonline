...
def test_percent_redistribution_single_zero():
    ...
    water = BillComponent(month_id=1, name="Water", amount=120.0, split_method='equal', position=1)
    water.id = 100
    ...
    calc = BillCalculator()
    contribs = calc.compute_contributions_dynamic(bill, [water], readings, p, [adj])
...
def test_amount_redistribution_overflow_normalized():
    ...
    readings = []
    internet = BillComponent(month_id=1, name="Internet", amount=100.0, split_method='equal', position=1)
    internet.id = 101
    ...
    calc = BillCalculator()
    contribs = calc.compute_contributions_dynamic(bill, [internet], readings, p, [adj])
...
def test_mixed_missing_targets_leftover_equal():
    ...
    electricity = BillComponent(month_id=1, name="Electricity", amount=90.0, split_method='usage', position=1)
    electricity.id = 102
    ...
    calc = BillCalculator()
    contribs = calc.compute_contributions_dynamic(bill, [electricity], readings, p, [adj])
    ...
    amounts = {c.participant.name: c.components["Electricity"] for c in contribs}
    assert round(amounts['Alice'], 2) == 60.0
    assert round(amounts['Bob'], 2) == 30.0
    assert round(amounts['Charlie'], 2) == 0.0
    assert sum(a.components["Electricity"] for a in contribs) == bill.electricity_amount
